...
def play():
    ...
    label_gagnant.config(text=result)
...
# 1. Créez un champ de saisie pour afficher le résultat du jeu. Liez-le à la variable result
label_gagnant = tk.Label(fenetre, text="result", font=("Arial", 14), bg="#f0f8ff")    
label_gagnant.grid(row=3, column=0)

label_bouton = tk.Label(fenetre)
label_bouton.grid(row=4)

# 2.Définissez un bouton <<jouer>> pour lancer le jeu lorsque vous cliquez dessus. Associez-le à la fonction play()
bouton_jouer = tk.Button(label_bouton, text="Jouer", command=play, bg="#4CAF50", fg="white")

# 3.Définissez un bouton <<reset>> pour réinitialiser le jeu lorsqu'on clique dessus. Associez-le à la fonction reset()
bouton_reset = tk.Button(label_bouton, text="Reset", command=reset, bg="#f44336", fg="white")

# 4.Définissez un bouton <<exit>> pour quitter l'application après un clic. Associez-le à la fonction exit()
bouton_exit = tk.Button(label_bouton, text="Exit", command=exit, bg="#FF9800", fg="white")

# 5.Placez les boutons et le champ de saisie des résultats à leurs positions respectives sur la fenetre de jeu
bouton_jouer.pack(side='left', padx=5)
bouton_reset.pack(side='left', padx=5)
bouton_exit.pack(side='left', padx=5)
label_gagnant.grid(row=3, column=0)

# 6.Executez l'application en appelant root.mainloop() pour démarrer le jeu Pierre, Papier, Ciseaux
fenetre.mainloop()